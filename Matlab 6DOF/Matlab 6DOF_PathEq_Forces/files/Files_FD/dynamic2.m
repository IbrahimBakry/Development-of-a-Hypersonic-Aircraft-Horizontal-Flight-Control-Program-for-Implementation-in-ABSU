...
% dydt(15) = V1*cos(p2);
% dydt(16) = V1*sin(p2)*sin(p3)/(p1+rearth);
% dydt(17) = -V1*sin(p2)*cos(p3)/((p1+rearth)*cos(phi1));

% dydt(15) = p1;
% dydt(16) = V1*sin(p2)*cos(chi1)/(h1+rearth);
% dydt(17) = V1*cos(gamma1)*cos(p3)/((h1+rearth)*cos(phi1));
